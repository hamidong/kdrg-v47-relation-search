...
def main():
    ...
    check("공개 AADRG 검색옵션",'<option value="AADRG">AADRG</option>' in html,True)
    check("공개 ADRG 검색옵션 제거",'<option value="ADRG">ADRG</option>' not in html,True)
    ...
    check("CODE 상세 AADRG 중심","function renderCodeDetail" in app and "'관련 AADRG'" in app,True)
    ...
    check("formatter public type","const ordered = ['CODE', 'AADRG'];" in fmt,True)
    check("relation AADRG 중심","makeBadge('AADRG')" in app,True)
